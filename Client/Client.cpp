...
int main()
{
    ...
    const char* sendBuffer1 = "Хай";//буфер1 для отправки данных
    const char* sendBuffer2 = "Не хай";//буфер2 для отправки данных
    ...
    if (result != 0)
    {
        cout << "getaddrinfo завершился ошибкой:" << result << endl;
    ...
    if (result == SOCKET_ERROR)
    {
        cout << "Сбой в отправке сообщения: " << result << endl;
        closesocket(ConnectSocket);
        freeaddrinfo(addrResult);
        WSACleanup();
        return 1;
    ...
    do// получение данных от сервера в цикле, если данные успешно получены то выводится result > 0, выводится количество байтов и сами данные; если соединения закрыто result = 0;
    {
        ...
        if (result > 0)
        {
            cout << "Полученно " << result << " bytes" << endl;
